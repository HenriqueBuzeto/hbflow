...
export default function AgentDetailPage({ params }: PageProps) {
  ...
  if (!agent) {
    return (
      <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md mx-auto space-y-4">
        ...
        <Link href="/agentes" className="text-xs font-bold text-primary hover:underline block">Voltar ao Catálogo</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12 animate-in fade-in duration-200">
      ...
      {/* Back Link */}
      <Link href="/agentes" className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 transition-colors w-fit">
        <ArrowLeft size={14} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-primary/10 text-primary rounded-xl border dark:border-slate-850">
                <Bot size={20} />
              </div>
              <div>
                <h2 className="text-sm font-bold text-slate-850 dark:text-slate-100">{agent.name}</h2>
                <span className="text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border dark:border-slate-850 px-2 py-0.5 rounded-full font-bold uppercase block mt-1 w-fit">
                  {agent.plan}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-3">
            ...
            <div className="space-y-1.5">
              {agent.triggers.map((trigger) => (
                <div key={trigger} className="text-[10px] font-mono bg-slate-50 dark:bg-slate-800/40 border dark:border-slate-850 p-2 rounded-lg text-slate-600 dark:text-slate-400">
                  ⚡ {trigger}
                </div>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-3">
            ...
            <div className="bg-slate-50 dark:bg-slate-800/20 border dark:border-slate-850 rounded-xl p-3 text-[10.5px] text-slate-500 font-mono leading-relaxed whitespace-pre-wrap">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
            ...
            <form onSubmit={handleTest} className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-850 rounded-2xl p-3 text-xs outline-none focus:border-primary text-slate-700 dark:text-slate-200"
                />
              </div>
            ...
            {testResult && (
              <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-800/30 border dark:border-slate-850 rounded-2xl animate-in fade-in duration-200">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-2.5 max-h-96 overflow-y-auto scrollbar-thin">
              {logs.length === 0 ? (
                ...
                logs.map((log) => (
                  <div key={log.id} className="p-3 border dark:border-slate-850 rounded-xl bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 transition-colors text-xs flex justify-between items-center">
                    <div>
